...
namespace GameFoundation.MetaProgression
{
    ...
    public sealed class DayCycleService : MonoBehaviour
    {
        ...
        private void ConsumeFood(){var humans=Find("Human");if(humans==null||GlobalResourceManager.Instance==null)return;var need=GlobalResourceManager.Instance.GetResourceAmount(humans);foreach(var foodName in new[]{"Bread","Berry"}){var food=Find(foodName);var take=Mathf.Min(need,food==null?0:GlobalResourceManager.Instance.GetResourceAmount(food));if(take>0){GlobalResourceManager.Instance.TrySpendResource(food,take);need-=take;}}if(need>0 && GlobalResourceManager.Instance.TrySpendResource(humans,need))DayResourceLedger.RecordStarvation(need);}
